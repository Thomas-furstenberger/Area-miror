...
export async function registerRoutes(
  ...
  fastify.get('/about.json', {
    schema: {
      tags: ['Services'],
      summary: 'Available services information',
      description: 'Get comprehensive information about all available services, their actions and reactions. This endpoint provides the service catalog required by the mobile client.',
      response: {
        200: {
              ...
              properties: {
                ...
                services: {
                  ...
                  items: {
                    type: 'object',
                    properties: {
                      name: { type: 'string', description: 'Service name (e.g., GitHub, Discord, Google)' },
                      actions: {
                        type: 'array',
  ...
  fastify.post('/api/auth/register', {
    schema: {
      ...
      description: 'Create a new user account with email and password. Returns access token and session token for immediate authentication.',
      body: {
        type: 'object',
      ...
      response: {
        201: {
          ...
          properties: {
            ...
            sessionToken: { type: 'string', description: 'Session token for maintaining user session' },
  ...
  fastify.post('/api/auth/login', {
    schema: {
      tags: ['Authentication'],
      summary: 'User login',
      description: 'Authenticate a user with email and password credentials. Returns access token and session token.',
      body: {
        type: 'object',
        required: ['email', 'password'],
        properties: {
  ...
  fastify.get('/api/auth/user', {
    schema: {
      ...
      description: 'Retrieve authenticated user information using the session token. Requires authentication via Bearer token in Authorization header.',
      security: [{ bearerAuth: [] }],
      response: {
        200: {
          ...
          properties: {
            ...
            name: { type: 'string', nullable: true, description: 'User display name' },
            avatarUrl: { type: 'string', nullable: true, description: 'User avatar URL from OAuth provider' }
          }
        },
        401: {
  ...
  fastify.post('/api/auth/logout', {
    schema: {
      tags: ['Authentication'],
      summary: 'User logout',
      description: 'Logout the current user by invalidating their session token. Requires authentication.',
      security: [{ bearerAuth: [] }],
      response: {
        200: {
  ...
  fastify.get('/api/auth/github', {
    schema: {
      ...
      description: 'Redirect to GitHub for OAuth2 authentication. Can be used for login or to link GitHub account to existing user. Pass optional state parameter with user token to link accounts.',
      querystring: {
        type: 'object',
        properties: {
          token: { type: 'string', description: 'User session token for account linking (optional)' },
  ...
  fastify.get('/api/auth/gmail/callback', {
    schema: {
      tags: ['OAuth'],
  ...
  fastify.get('/api/user/oauth-accounts', {
    schema: {
      tags: ['Authentication'],
  ...
  fastify.delete('/api/user/oauth/:provider', {
    schema: {
      tags: ['Authentication'],
  ...
  // --- AREA MANAGEMENT ---
  fastify.post('/api/areas', {
    schema: {
      tags: ['Areas'],
      summary: 'Create automation (AREA)',
      description: 'Create a new automation workflow connecting an action (trigger) from one service to a reaction from another service. For example: "When I receive an email on Gmail, send a Discord notification".',
      security: [{ bearerAuth: [] }],
      body: {
        type: 'object',
        ...
        properties: {
          ...
          actionType: {
            type: 'string',
            description: 'Type of trigger action (e.g., "new_email", "new_commit", "new_message")'
          },
          actionConfig: {
            type: 'object',
            ...
            areas: {
              ...
              items: {
                ...
                properties: {
                  ...
                  reactionService: { type: 'string', description: 'Service executing the reaction' },
                  reactionType: { type: 'string', description: 'Type of reaction' },
                  reactionConfig: { type: 'object', description: 'Reaction configuration' },
                  lastTriggered: { type: 'string', nullable: true, format: 'date-time', description: 'Last execution timestamp' },
                  createdAt: { type: 'string', format: 'date-time' },
                  updatedAt: { type: 'string', format: 'date-time' }
                }
              }
  ...
  fastify.put('/api/areas/:id/toggle', {
    schema: {
      tags: ['Areas'],
      summary: 'Toggle automation status',
      description: 'Enable or disable an automation workflow. When disabled, the automation will not execute even if the trigger condition is met.',
      security: [{ bearerAuth: [] }],
      params: {
        type: 'object',
        required: ['id'],
  ...
  fastify.post('/api/area/gmail/send_email', {
    schema: {
      tags: ['Services'],
  ...
  fastify.get('/api/auth/spotify/callback', {
    schema: {
      tags: ['OAuth'],
